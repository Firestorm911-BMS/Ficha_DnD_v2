...
**Fecha de última actualización**: 2026-05-25 (sesión 10)
**Rama activa**: `main`
...
| 2026-05-24 | BUG-28 | Botón "Aplicar" dañaba al jugador en vez del enemigo | dice.js: los dos `addCombatLog` de daño de ataque (rollAttackDamage + _showDamageAnimation) usaban `applyDamageAmount` que baja PG del jugador. Cambiado a `window.applyTrackerDamage(total, options, this)`. initiative.js: nueva función `applyTrackerDamage` — si 0 no-player entries: aviso en log; si 1 entry: aplica directamente; si 2+: muestra selector flotante `_showTargetPicker`. El botón pasa a llamarse "→ Aplicar al objetivo". |
...
| 2026-05-24 | FEAT-14 | Optimización CSS mobile/web — legibilidad y táctil | **styles.css** (19 edits inline): todos los `font-size: 8px` y `font-size: 9px` de etiquetas funcionales subidos a `10px` mínimo. Selectores: `.font-btn-name`, `.meta-label` (base + media 768px), `.btn-sm`, `.attr-name`, `.stat-hex-label`, `.hp-temp-label`, `.skill-attr`, `.death-label`, `.attacks-table th`, `.attack-meta`, `.attack-state-tag`, `.attack-props`, `.attack-rage-tag`, `.attack-mini-btn`, `.attack-edit-grid label`, `.dc-label`, `.condition-tag`, `.resource-name`, `.hero-pill-label`. **refresh.css** (nueva sección §24): `.btn { min-height: 34px }`, `.btn-sm { min-height: 28px }`, `.skill-prof`/`.save-prof` con `::before { inset: -10px }` para área táctil ampliada sin cambio visual, `.death-check` 16px→20px, dice-panel full-width en ≤600px, card padding 16px→12px en ≤768px, tab-btn min-height 40px + btn-sm ajustado en ≤480px. |
| 2026-05-24 | FEAT-14b | Segundo pass CSS — todos los font-size < 11px unificados a 11px | **styles.css**: PowerShell global replace de todas las instancias restantes de `font-size: 8px` / `font-size: 9px` → `11px`, luego `font-size: 10px` → `11px`. Resultado final: 0 instancias de font-size menor a 11px en el proyecto completo. **refresh.css**: misma operación. commits f74c8ed, 303a3cb. |
| 2026-05-24 | BUG-25 | Usos de recurso de clase no persisten al recargar | **rage.js**: `toggleRage()`, `toggleRagePip()`, `toggleExtraResourcePip()` cambiados de `window.saveToLocal?.()` (debounce 500ms) a `window.saveState?.()` (inmediato). El debounce causaba pérdida de usos si el usuario recargaba antes de 500ms, y en browsers mobile `beforeunload` no es confiable. Solución: acciones discretas de estado (activar/usar recurso) guardan síncronamente. |
| 2026-05-24 | FEAT-15b | Lore de armas — fila separada colspan="4" | **attacks.js** `renderAttacks()`: el lore ya no va dentro del primer `<td>` sino en una `<tr class="attack-lore-row">` adicional con `<td colspan="4" class="attack-lore-cell">`. Esto da al lore la misma anchura total de la tabla, igual que el lore de conjuros en su tarjeta. **styles.css**: nuevos selectores `.attack-lore-row`, `.attack-lore-cell`; `.attack-lore-text` y `.spell-lore-text` ahora tienen `padding: 6px 10px`, `border-left: 2px solid var(--gold-dark)`, `background: rgba(201,168,76,0.04)` para aspecto de cita/nota. |
| 2026-05-24 | refactor | Ataques: tabla → tarjetas div, lore con ancho completo | **attacks.js** `renderAttacks()`: `<tr>/<td>` reemplazados por `<div class="attack-card">` con grid interno. El lore usa `<div class="attack-lore-wrap">` con `grid-column: 1 / -1` para abarcar el 100% del ancho (el `colspan="4"` era ignorado por `display:grid` del CSS anterior). **index.html**: `<table class="attacks-table">/<thead>/<tbody>` → `<div class="attacks-list" id="attacksList">`. **styles.css**: selectores `.attacks-table tr` → `.attack-card`, `.attack-col-name/bonus/damage/delete`, `.attack-lore-wrap`. Eliminados `.attack-lore-row` y `.attack-lore-cell`. commits ca193ab (intento CSS previo), b034423 (refactor final). |
| 2026-05-24 | feat | Botones ▲▼ para reordenar armas | **attacks.js**: `moveAttack(i, dir)` — intercambia `state.attacks[i]` con `[i+dir]`, re-renderiza y guarda. Botones ▲▼ en `.attack-col-delete` con `disabled` en extremos. `window.moveAttack` bridge. **styles.css**: `.attack-col-delete` → `flex-column`; `.attack-sort-btn` ghost con hover dorado y `:disabled` opaco; último column del grid 28px → 34px. commit 73e122c. |
| 2026-05-24 | fix | Backstory — resumen más grande y menos padding | **styles.css**: `.attack-lore-summary` font-size 11px → 13px; `.attack-lore-wrap` padding-top 2px → 0 + `margin-top: -4px` para pegar el bloque al contenido del arma. commit 21f3789. |
| 2026-05-24 | fix | Renombrar "Lore / Historia" → "Backstory" | Etiqueta del `<summary>` en tarjetas de armas (`attacks.js`) y conjuros (`spells.js`), y labels de textareas en los modales de arma y conjunto (`index.html`). commit 3f38120. |
| 2026-05-25 | FEAT-20 | Estandarizar distancias: metros → pies (PHB oficial) | **app.js** `SPECIES_DATA`: 15 entradas actualizadas (speed `'9m'→'30 ft'`, `'7.5m'→'25 ft'`, `'10.5m'→'35 ft'`; sight `'18m'→'60 ft'`, `'36m'→'120 ft'`). **species.json**: speeds numéricos (7.5→25, 9→30, 10.5→35), vision strings, traits del Drow (36m→120 ft), rasgo del Elfo de Bosques, armas de soplo de los 10 ancestros del Dracónido (línea `1.5×9m`→`5×30 ft`, cono `4.5m`→`15 ft`). **wizard.js**: sufijo display `'m'→' ft'` en panel de especie (línea 456) y resumen final (línea 1242); `set('statSpeed', speed)` → `speed + ' ft'` (línea 1470). **persistence.js**: migración v3→v4 — convierte `statSpeed` de metros a pies usando tabla de valores PHB conocidos + conversión genérica `×3.28/5×5` para el patrón `"Xm"`. **inventory.js**: footer label `'+N vel.'` → `'+N ft vel.'`. **tests.html**: 7 tests de migración v4 (valores "9m", "9", "7.5m", "10.5m", ya migrado, null, re-ejecución). |
| 2026-05-25 | FEAT-19 | Botones ▲▼ para reordenar equipamiento | **inventory.js**: `moveInventoryItem(i, dir)` — intercambia `state.inventory[i]` con `[i+dir]`, re-renderiza y guarda. Botones ▲▼ en `.inv-sort-col` con `disabled` en extremos. `window.moveInventoryItem` bridge. **styles.css**: `grid-template-columns` de `.inv-item` añade una columna `auto` extra; nuevos selectores `.inv-sort-col` (flex-column, gap 1px) y `.inv-sort-btn` (ghost, hover dorado, `:disabled` opaco). |
| 2026-05-25 | fix | Condition tags desbordan su celda en nombres largos | **styles.css**: `.conditions-grid` minmax 100px → 114px (espacio suficiente para "INCAPACITADO"/"INCONSCIENTE", los nombres más largos). `.condition-tag` letter-spacing 1px → 0.5px (reduce ~6px el ancho del texto). Añadido `overflow:hidden; white-space:nowrap; text-overflow:ellipsis` como fallback. commit 54f4de1. |
| 2026-05-25 | FEAT-18b | Agotamiento Nv.4: clampear PG actuales al nuevo máximo | **conditions.js** `changeExhaustion`: al llegar a Nv.4+, si `hpCurrent > max/2` se reduce a `max/2` con log de combate. Idempotente — si los PG ya estaban por debajo no cambia nada. 2 tests nuevos: clampeo (23/26→13/13) y caso sin clampeo (5/26→5). |
| 2026-05-25 | FEAT-18 | Efectos de Agotamiento RAW (PHB 5e p.291) | **hp.js**: `updateHP` y `syncCombatOverlay` calculan `effectiveMax = max/2` en Nv.4+; `updateHP` agrega/quita `exhaustion-reduced-stat` en `#hpMax` (visual, no modifica el valor). **conditions.js**: `changeExhaustion` llama `window.updateHP?.()` y `window.__syncBattleStance?.()` para actualizar en tiempo real. **app.js** `syncBattleStance`: lee velocidad base de `state.CHARACTER_STATE.statSpeed` (no del DOM) para evitar corrupción; calcula `effectiveSp` (Nv.2+ → mitad, Nv.5+ → 0); actualiza solo `#bsSpeed` con clase visual — `#statSpeed` (valor guardado) nunca se toca. **refresh.css**: `exhaustion-reduced-stat` (rojo pulsante + tachado), `pipActiveGlow` para pips de ranuras activas. **tests**: 5 tests RAW nuevos + elementos mock `statSpeed`, `bsSpeed`, `bsHpMax`, `hpBar`, `hpStatus`. **serve.ps1**: servidor HTTP PowerShell nativo como alternativa a `npm run serve`. |
| 2026-05-25 | FEAT-17 | Campo `baseWeapon` — registra el preset original del arma | **attacks.js**: `WEAPON_TYPE_MAP` (nombre ES → clave SVG); `getWeaponKey(atk)` exportado (prioriza `baseWeapon` sobre `name`); `normalizeAttack` añade `baseWeapon: ''`; `renderAttacks` inyecta `.attack-card-bg-icon` (SVG watermark via `window.WEAPON_SVGS`) y `.attack-base-tag` (badge con nombre del preset); `openAttackModal`/`saveAttackFromModal` leen/escriben `#amBaseWeapon`; `onAttackPresetChange` setea `#amBaseWeapon` al nombre del preset o lo borra si se elige "Personalizada"; bridge `window.getWeaponKey`. **dice.js**: `_weaponKey(atk)` usa `atk.baseWeapon \|\| atk.name` como fuente; `window.WEAPON_SVGS` expuesto desde el IIFE; `rollAttack` pasa `weaponSvg: WEAPON_SVGS[_weaponKey(atk)]` a `LL_cinematicRoll`. **index.html**: `<input type="hidden" id="amBaseWeapon">` en modal de arma. **styles.css**: `.attack-card` añade `position:relative; overflow:hidden`; `.attack-card-bg-icon` (absolute, opacity 0.06, 62px); `.attack-base-tag` (italic dorado, borde sutil). |
| 2026-05-24 | FEAT-16 | Modificador de atributo dinámico en daño de armas | Campo `addAbilityMod: boolean` en el ataque. Cuando es `true`, las tiradas de daño suman `getMod(ability)` dinámicamente en vez de depender del bono bakeado en el string. `normalizeAttack()`: `addAbilityMod ?? false` (backward-compat, armas existentes no cambian). Modal: checkbox "Sumar mod. al daño" (checked por defecto en armas nuevas y presets PHB). `_updateAttackPreview()`: bono condicional. `renderAttacks()`: helper `_dmgDisplay()` muestra el modificador en vivo en la tarjeta. `dice.js`: 4 sitios actualizados (`showDamagePrompt`, `rollAttackDamage`, `_showDamageAnimation`, `rollAttack` onComplete). |
| 2026-05-24 | BUG-26 | Animaciones de dados se cerraban solas | **dice.js**: eliminados los tres auto-cierre de `rollStage`: (1) `rollPolyhedral` → `setTimeout(remove, 2500)`; (2) `_showDamageAnimation` → `setTimeout(remove, 3200)`; (3) `LL_cinematicRoll` → `_stageDur` por defecto cambiado de `2000/2900ms` a `0`. Las animaciones ahora permanecen hasta que el usuario hace clic en el overlay para cerrarlas, igual que las tiradas de ataque (`stageDuration: 0` ya existía). El caller puede pasar `stageDuration > 0` si quiere auto-cierre explícito. |
| 2026-05-24 | FEAT-15 | Lore / Historia en armas y conjuros | Campo `lore` (string, default `''`) añadido a ataques y conjuros. **index.html**: textarea `#amLore` en attack modal y textarea `#smLore` en spell modal. **attacks.js**: `normalizeAttack()` incluye `lore: atk.lore \|\| ''`; `openAttackModal()` carga lore en el textarea; `saveAttackFromModal()` persiste lore; `renderAttacks()` renderiza `<details class="attack-lore-details">` con `<summary>📖 Lore</summary>` si el campo tiene contenido. **spell-modal.js**: `openSpellModal()` carga `spell?.lore`; `saveSpellModal()` persiste `lore`. **spells.js**: `buildSpellCard()` añade bloque `<details class="spell-lore-details">` entre `.spell-card-desc` y `.spell-card-actions` cuando hay lore. **persistence.js**: carga de conjuros ahora normaliza con `.map(s => ({ ...s, lore: s.lore \|\| '' }))`. **styles.css**: nuevos selectores `.attack-lore-details`, `.spell-lore-details`, `.attack-lore-summary`, `.spell-lore-summary`, `.attack-lore-text`, `.spell-lore-text` (cursiva, color secondary, `white-space: pre-wrap`). **tests.html**: 3 tests FEAT-15 en sección nueva + `#amLore`/`#smLore` en DOM stub. |
